object day12 {
    class moon (px:Int, py:Int, pz:Int, vx:Int, vy:Int, vz:Int){
        ...
        def this(position:List[Int], velocity:List[Int]){
            this(position(0), position(1), position(2), velocity(0), velocity(1), velocity(2))
        }
        def position():List[Int]={List(px, py, pz)}
        def velocity():List[Int]={List(vx, vy, vz)}

        def updateV(update:List[Int]):moon={new moon(this.position(), (this.velocity.zip(update).map({case (a,b)=>a+b})))}
        def updateP(update:List[Int]):moon={new moon(this.position().zip(update).map({case (a,b)=>a+b}), this.velocity())}
    ...
    val smallMoons=List(new moon(List(-1,0,2),List(0,0,0)), new moon(List(2,-10,-7),List(0,0,0)), new moon(List(4,-8,8),List(0,0,0)), new moon(List(3,5,-1), List(0,0,0)))
    ...
    scala.collection.Iterator.iterate(smallMoons)(update).drop(2772).next.map(x=>println(x.position))
    val resultList =scala.collection.Iterator.iterate(allMoons)(update).drop(1000).next
    ...
    scala.collection.Iterator.iterate(update(allMoons))(update).takeWhile(_.map(x=>x.toxTup())!=temp).size
    def cont(allMoons:List[moon], steps:Long, history:Set[List[(Int,Int)]]):Long={
        val temp = allMoons.map(x=>x.toxTup)
        if (history(temp)) {
    ...
    def cont2(allMoons:List[moon], steps:Int, history:Set[List[(Int,Int)]]):Int={
        val temp = allMoons.map(x=>x.toyTup)
        if (history(temp)) steps+1 else cont2(update(allMoons), steps+1, history incl temp)
    }
    def cont3(allMoons:List[moon], steps:Int, history:Set[List[(Int,Int)]]):Int={
        val temp = allMoons.map(x=>x.tozTup)
        if (history(temp)) steps+1 else cont3(update(allMoons), steps+1, history incl temp)
    }
