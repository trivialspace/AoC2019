...
object Main extends App {
  ...
  val mytest: Vector[Int] =
    "2,4,4,5,99,0".split(",").map(_.trim()).toList.map(_.toInt).toVector
  ...
  def update(position: Int, memory: Vector[Int]): Vector[Int] = {
    if (memory(position) == 99) {
      ...
      update(
        position + 4,
        memory.updated(memory(position + 3), computeValue(position, memory))
      )
    }
  }
  ...
  def try2(noun: Int, verb: Int): Int = {
    println(noun)
    println(verb)
    if (update(0, myvec.updated(1, noun).updated(2, verb))(0) == 19690720)
      100 * noun + verb
    else try2(mygen.nextInt(100), mygen.nextInt(100))
  }
