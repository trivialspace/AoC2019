...
val dir1 = input1.split(",").map(_.trim()).toList.map(e=>repeatChar(e)).mkString
val dir2 = input2.split(",").map(_.trim()).toList.map(e=>repeatChar(e)).mkString
