object day9 {
  ...
  def three(myvec: Vector[Long], position: Int, input: Int): Vector[Long] = {
    myvec.updated(position, input)
  }
  def four(myvec: Vector[Long], position: Int): Vector[Long] = {
    println(myvec(position))
    myvec
  }
  def five(myvec: Vector[Long], current: Int, pos2: Int): Vector[Long] = {
  ...
  def seven(
    ...
    if (myvec(pos1) < myvec(pos2)) myvec.updated(final_pos, 1)
    else myvec.updated(final_pos, 0)
  }
  def eight(
      myvec: Vector[Long],
      pos1: Int,
  ...
  def parser(opcode: Long): (Int, List[Int]) = {
    // val operation = opcode%100
    (opcode.toInt % 100, (opcode / 100).toString.map(_.asDigit).reverse.toList)
  }
  def get_pose(
      base: Int,
      mode: Option[Int],
    ...
    mymode match {
      case 1 => position
      case 0 => myvec(position).toInt
      case 2 => myvec(position + base).toInt
    }
  }
    ...
    opcode match {
      ...
      case 1 =>
        update(
          one(
            myvec,
            get_pose(base, mode = params.lift(0), myvec, position + 1),
            get_pose(base, mode = params.lift(1), myvec, position + 2),
            get_pose(base, mode = params.lift(2), myvec, position + 3)
          ),
          position + 4,
          input = input,
          base = base
      ...
      case 3 => {
        val h :: t = input
        update(
          three(myvec, get_pose(base, params.lift(0), myvec, position + 1), h),
          position + 2,
          t,
          base = base
        )
      ...
      case 4 =>
        update(
          four(myvec, get_pose(base, params.lift(0), myvec, position + 1)),
          position + 2,
          input = input,
          base = base
        ) //Vector(myvec(get_pose(base, params.lift(0), myvec, position+1)))
      case 5 =>
        if (myvec(get_pose(base, params.lift(0), myvec, position + 1)) != 0)
          update(
            myvec,
            myvec(get_pose(base, params.lift(1), myvec, position + 2)).toInt
          )
        else update(myvec, position + 3, input = input, base = base)
      case 6 =>
        if (myvec(get_pose(base, params.lift(0), myvec, position + 1)) == 0)
          update(
            myvec,
      ...
      case 7 =>
        update(
          seven(
            myvec,
            get_pose(base, params.lift(0), myvec, position + 1),
            get_pose(base, params.lift(1), myvec, position + 2),
            get_pose(base, params.lift(2), myvec, position + 3)
          ),
          position + 4,
          input = input,
          base = base
  ...
  val small = "109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99"
  val smallvec = small
    .split(",")
    .map(_.trim())
    .toList
  ...
  update(smallvec, 0, List(0), 0)
  update(myvec, 0, List(1), 0)
  Vector.fill(10)(5)
}
