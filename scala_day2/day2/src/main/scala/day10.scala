...
object day10 {
  ...
  def counter(asteroids: List[(Int, Int)], x: Int, y: Int): Int = {
    ...
    val allVert = asteroids
      .filter(_._1 == x)
      .map({ case (a, b) => scala.math.signum(scala.math.subtractExact(b, y)) })
      .toSet
      .size
    noVert + allVert
  }
  ...
  def viz3(x: Int, y: Int, smallAsteroids: List[(Int, Int)]): Map[Float, List[(Int, Int)]] = {
    (for ((a, b) <- smallAsteroids)
      yield ((b - y).toFloat / (a - x) -> (a - x, b - y))).groupMap(_._1)(_._2)
  }
  ...
  val right = scala.collection.immutable.List(
    temp2
      .filter({ case (_, b) => ((b.head._1 > 0) || (b.head._1 == 0 && b.head._2 > 0)) })
      .toSeq
      .sortWith(_._1 > _._1): _*
  )
  val left = scala.collection.immutable.List(
    temp2
      .filter({ case (_, b) => ((b.head._1 < 0) || (b.head._1 == 0 && b.head._2 < 0)) })
      .toSeq
      .sortWith(_._1 < _._1): _*
