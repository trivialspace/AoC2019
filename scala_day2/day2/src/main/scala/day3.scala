object day3 {
  ...
  class line(raw: String) {
    val direction = raw.take(1)
    val length = raw.drop(1)
  }

  case class point(x: Int, y: Int, delay: Int = 0)

  def repeatChar(dir: String): String = {
    dir.take(1) * dir.drop(1).toInt
  }

  val dir1 =
    input1.split(",").map(_.trim()).toList.map(e => repeatChar(e)).mkString
  val dir2 =
    input2.split(",").map(_.trim()).toList.map(e => repeatChar(e)).mkString
  ...
  def process(dir: String, current: point, marks: List[point]): List[point] = {
    dir.take(1) match {
      ...
      case "R" =>
        process(
          dir.drop(1),
          new point(current.x + 1, current.y, current.delay + 1),
          new point(current.x + 1, current.y, current.delay) :: marks
        )
      case "L" =>
        process(
          dir.drop(1),
      ...
      case "U" =>
        process(
          dir.drop(1),
          new point(current.x, current.y + 1, current.delay + 1),
          new point(current.x, current.y + 1, current.delay) :: marks
        )
      case "D" =>
        process(
          dir.drop(1),
  ...
  val inter = (for {
    p <- test1; q <- test2
    if (p.x == q.x && p.y == q.y)
  } yield p.delay + q.delay).filter(t => t > 0).min
