object day5 {
  ...
  val myvec: Vector[Int] =
    input.split(",").map(_.trim()).toList.map(_.toInt).toVector
    ...
    opcode match {
      ...
      case 1 =>
        update(
          one(
            myvec,
            get_pos(mode = params.lift(0), myvec, position + 1),
            get_pos(mode = params.lift(1), myvec, position + 2),
            get_pos(mode = params.lift(2), myvec, position + 3)
          ),
          position + 4,
          input = input
        )
      ...
      case 3 => {
        val h :: t = input
        update(
          three(myvec, get_pos(params.lift(0), myvec, position + 1), h),
          position + 2,
          t
        )
      }
      case 4 =>
        Vector(myvec(get_pos(params.lift(0), myvec, position + 1))) //update(four(myvec, get_pos(params.lift(0), myvec, position+1)), position+2)
      case 5 =>
        if (myvec(get_pos(params.lift(0), myvec, position + 1)) != 0)
          update(myvec, myvec(get_pos(params.lift(1), myvec, position + 2)))
        else update(myvec, position + 3, input = input)
      case 6 =>
        if (myvec(get_pos(params.lift(0), myvec, position + 1)) == 0)
          update(myvec, myvec(get_pos(params.lift(1), myvec, position + 2)))
        else update(myvec, position + 3, input = input)
      case 7 =>
        update(
          seven(
            myvec,
            get_pos(params.lift(0), myvec, position + 1),
            get_pos(params.lift(1), myvec, position + 2),
            get_pos(params.lift(2), myvec, position + 3)
          ),
          position + 4,
          input = input
        )
